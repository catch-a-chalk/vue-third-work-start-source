export default class FetchProvider {
  ...
  request (options) {
    ...
    return fetch(options.baseUrl + options.path + this.computeQueryParams(options.query),
  { headers: options.headers, bofy, method: options.method })
  .then((response) => {
    if (!response.ok) {
  ...
  // Метод добавления перехватчиков
